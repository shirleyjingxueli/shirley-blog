...
4. **安全性：**
  - GET 请求参数在 URL 上，相比于 POST 请求来说不是很安全，因为请求的 URL 会被保存在历史记录中。  
5. **请求长度：**
...
### OSI 七层模型
  ...
  5. 网络层
    ...
    本层通过 IP 寻址来建立两个节点之间的连接，为源端的运输层送来的分组选择合适的路由和交换节点，正确无误的按照地址传送给目的端的运输层。
...
#### 三次握手
  ...
  实质上就是连接服务器指定端口，建立 TCP 连接，并同步连接双方的序列号和确认号，交换 TCP 窗口大小信息。
