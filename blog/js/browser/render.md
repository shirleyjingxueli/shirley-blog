...
### 布局阶段
  ...
  2. 布局计算：计算布局树节点的坐标位置（相对于其包含块）。

  **DOM树 和 layout 树不是一一对应的。比如：隐藏块，伪元素，行盒不能和块盒在一起，内联元素必须包含在行盒中。**
...
### 为什么较少重绘、重排能优化Web性能？有哪些方法能够减少重绘、重排？
  ...
  - 将元素先设置为 display:none, 操作结束后再把它显示出来。因为在 display 属性为 none 的元素上进行 DOM 操作并不会引发回流和重绘。
  - 避免使用table布局，一个小的改动可能会使整个 table 进行重新布局
  - 批量 dom 操作，例如 createDocumentFragment, 或者使用框架，比如React
  - Debounce window resize 事件
  - 对 dom 属性的读写要分离：将 DOM 的多个读操作（或者写操作）放在一起，而不是读写操作穿插着写。这得益于浏览器的渲染队列机制。
  - will-change:transform做优化。

  **浏览器针对页面的回流和重绘，进行了自身的优化--渲染队列**

  **浏览器会将所有的回流、重绘的操作放在一个队列中，当队列中的操作到了一定数量或者到了一定的时间间隔，浏览器会对队列进行批处理。这样就会让多次的回流、重绘变成一次回流和重绘。**

  **上面讲多个读操作或者写操作放在一起，就会等所有的读操作进入队列之后执行，这样，原本应该是触发多次回流，变成了只触发一次回流。**
